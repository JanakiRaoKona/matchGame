...
class App extends Component {
  ...
  onClickStart = () => {
    ...
    this.setState({matchImage: imagesList[0]})
  }
  ...
  getTheActiveTabsList = () => {
    const {activeTabsId} = this.state
    const activeTabsFilterList = imagesList.filter(
      eachImageList => eachImageList.category === activeTabsId,
    )
    return activeTabsFilterList
  }
  ...
  onClickThumbMatch = id => {
    ...
    if (id === matchImage.id) {
      this.setState({matchImage: imagesList[randomImageIndex]})
  ...
  render() {
    const {seconds, matchImage, score, showHide, activeTabsId} = this.state
    const activeTabsFilterList = this.getTheActiveTabsList()
            ...
            <li>
              <div className="end-content">
                ...
                <img
                  ...
                  alt="timer"
                />
                <p className="nav-seconds">{seconds} Sec</p>
              </div>
            </li>
          </ul>
        </nav>
            ...
            <ul className="tabs-container">
              {tabsList.map(eachTab => (
                <TabItem
                  ...
                  isActive={eachTab.tabId === activeTabsId}
                />
              ))}
            </ul>
            <ul className="thumbnail-images">
              {activeTabsFilterList.map(eachImage => (
                <ImageItem
                  imagesList={eachImage}
                  key={eachImage.id}
                  onClickThumbMatch={this.onClickThumbMatch}
                />
              ))}
            </ul>
          </div>
        ) : (
          <div className="score-card-bg1">
            <div className="score-card-bg">
              ...
              <p className="your-score-num">{score}</p>
              <button
                type="button"
